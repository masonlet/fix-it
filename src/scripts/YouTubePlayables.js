...
export const YouTubePlayables = {
  ...
  sendScore: function (score) {
    if (this.isReady()) {
      try {
        ...
        if (error.errorType) this.handleError(error.errorType);
      }
    }
  },

  setOnPause: function (callback) {
    if (this.isLoaded()) this._ytgameRef.system.onPause(callback);
  },

  setOnResume: function (callback) {
    if (this.isLoaded()) this._ytgameRef.system.onResume(callback);
  },
  ...
  logError: function () {
    if (this.isLoaded()) this._ytgameRef.health.logError();
  },

  logWarning: function () {
    if (this.isLoaded()) this._ytgameRef.health.logWarning();
  },
  ...
  isAudioEnabled: function () {
    if (this.isLoaded()) return this._ytgameRef.system.isAudioEnabled();
    else                 return true;
  },
